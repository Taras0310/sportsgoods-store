...
export default function Products() {
  ...
  const {queryFilters, currentCategoryObject, setQueryFilters, setCurrentCategoryObject} = useStore()
    ...
    <div >
     ...
     <div className='products-block'>
      {currentCategoryObject && (
        <>
          <SubcategoryList subcategories={currentCategoryObject.subcategories}  />
          <div className='search'>
          <select value={selectValue} onChange= {e => setSelectValue(e.target.value)} >
            <option disabled value=''>Сортувати за</option>
            <option value='brand'> Брендами</option>
            <option value='price'>Ціною</option>
          </select>
          <button className='clear' onClick={filterObj}>Очистити фільтр</button>
          </div>
